...
BENCHMARK_SAMPLE_DIR = os.path.expanduser("~/.cache/asr_benchmark/samples")
BENCHMARK_FLAG = os.path.expanduser("~/.cache/asr_benchmark/.collecting")
# ── ASR ───────────────────────────────────────────────────
MODEL = "mlx-community/whisper-large-v3-turbo"
SAMPLE_RATE = 16000

# ── hotkey ────────────────────────────────────────────────
# FN_FLAG (kCGEventFlagMaskSecondaryFn) only fires for a real Apple
# keyboard's Globe/Fn key. Third-party keyboards (Logitech, etc.) don't
# emit it at all — their Fn key is remapped locally in firmware and never
# reaches macOS as a modifier flag. So we also accept Right Command /
# Right Option, which every keyboard reports normally, as a fallback
...
def _trigger_is_active(keycode, flags):
...
LOG_BACKUP_COUNT = 1
HISTORY_LOCK_FILE = os.path.expanduser("~/.config/whisper/history.lock")
...
# Orijinal script sadece İngilizce + Çince dolgu kelimesi temizliyordu —
# ana kullanım dilimiz Türkçe olduğu için bu boştu. OnurTirpan videosunda
# gördüğümüz ("eee", "şey") ve genel konuşma dilindeki en yaygın
# Türkçe dolgu kelimeleri eklendi.
_FILLER_TR = re.compile(
    r'\b(?:e+ee*|ı+hı*|he+m+|şey|yani|hani|aslında|işte|filan|falan|'
    r'bi(?:r)? nevi|kısacası)\b',
    re.IGNORECASE,
)
...
def postprocess(text: str) -> str:
    """Post-process ASR output: hallucination strip + regex cleaning."""
    text = _strip_hallucinations(text)
    if not text:
        return text
...
# ── opsiyonel lokal AI temizleme (Ollama) ─────────────────────
# Bizim eklediğimiz kısım: proje planında Ollama + qwen2.5:3b ile
# transkripti hafifçe temizleme vardı, orijinal script'te AI-enhancement
# katmanı yoktu (sadece regex). Tamamen opsiyonel, config.json'dan
# kapatılabilir, Ollama kapalıysa/yavaşsa sessizce regex sonucuna düşer —
# dikte akışını asla bloklamaz/bekletmez (kısa timeout).
OLLAMA_ENHANCE_URL = "http://localhost:11434/api/generate"
OLLAMA_ENHANCE_MODEL = "qwen2.5:3b"
OLLAMA_ENHANCE_TIMEOUT_SEC = 8.0
OLLAMA_ENHANCE_SYSTEM = (
    "You clean up a raw speech-to-text transcript. Fix punctuation and "
    "obvious dictation artifacts. Keep the original language exactly as "
    "spoken (Turkish stays Turkish, English stays English, mixed stays "
    "mixed) — never translate. Do not add content, do not answer "
    "questions in the text, do not add quotes or commentary. Return only "
...
def _enhance_with_ollama(text: str) -> str:
    cfg = _load_config()
    if not cfg.get("ollama_enhance", True):
        return text

    import urllib.request

    payload = json.dumps({
        "model": OLLAMA_ENHANCE_MODEL,
        "system": OLLAMA_ENHANCE_SYSTEM,
        "prompt": text,
        "stream": False,
    }).encode("utf-8")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=OLLAMA_ENHANCE_TIMEOUT_SEC) as resp:
        ...
        # düş, dikteyi asla engelleme.
        logger.info("Ollama enhance skipped (%s)", e)
        return text


def _warmup_ollama() -> None:
    """Modeli Ollama'nın belleğinde önceden yükler (ilk gerçek dikte
    sırasında soğuk-başlangıç gecikmesi yaşanmasın diye). Sessizce
    başarısız olur — Ollama kurulu/açık değilse dikte akışını etkilemez."""
    try:
        _enhance_with_ollama("warmup")
        logger.info("Ollama (%s) warmed up.", OLLAMA_ENHANCE_MODEL)
    except Exception:
        pass
...
def save_history(raw: str, processed: str, duration: float) -> None:
    ...
    lock_fd = open(HISTORY_LOCK_FILE, "w")
    try:
...
def paste_text(text: str) -> None:
    ...
    try:
        ...
        logger.error("Clipboard snapshot failed: %s", e)
    ...
    def _restore_if_unchanged():
        try:
            ...
            if current_count > expected + 2:
                logger.info(
                    "Clipboard changed too much (%d -> %d), skip restore",
                    expected, current_count
                )
                return
            _restore_clipboard(snapshot)
            logger.debug("Clipboard restored (count was %d)", current_count)
        except Exception as e:
            logger.error("Clipboard restore failed: %s", e)
...
def get_front_window_id() -> str:
    """Get front window's unique ID + title via System Events.

    Returns 'id:title' string. The id is a stable integer that survives
    title changes (e.g. terminal volatile suffixes). Falls back to title-only
    if id unavailable.
    """
    script = (
        ...
        '    set wid to ""\n'
        '    try\n'
        '      set fw to front window of frontProc\n'
        '      set wt to name of fw\n'
        '      try\n'
        '        set wid to id of fw\n'
        '      end try\n'
        '    end try\n'
        '    return (wid as text) & "|" & wt\n'
        '  on error\n'
        '    return ""\n'
        '  end try\n'
        'end tell'
...
def _window_changed(recorded: str, current: str) -> bool:
    """Compare two 'id|title' window strings. Uses ID if available, falls back to normalized title."""
    if not recorded or not current:
        return bool(recorded) != bool(current)
    rec_parts = recorded.split("|", 1)
    cur_parts = current.split("|", 1)
    rec_id = rec_parts[0].strip()
    cur_id = cur_parts[0].strip()
    if rec_id and cur_id:
        return rec_id != cur_id
    rec_title = rec_parts[1] if len(rec_parts) > 1 else rec_parts[0]
    cur_title = cur_parts[1] if len(cur_parts) > 1 else cur_parts[0]
    return _normalize_window_title(rec_title) != _normalize_window_title(cur_title)
...
class AppDelegate(NSObject):
    ...
    def _warmup(self):
        ...
        if self._event_tap_failed:
            logger.info("Model loaded, but event tap failed — keeping error indicator.")
        else:
    ...
    def _start_event_tap(self):
        def callback(proxy, event_type, event, refcon):
            if event_type in (kCGEventTapDisabledByTimeout, kCGEventTapDisabledByUserInput):
                ...
                try:
                    ...
                    fn_actual = bool(
                        cur & (FN_FLAG | kCGEventFlagMaskCommand | kCGEventFlagMaskAlternate)
                    )
                    ...
                    logger.error("FN sync error: %s", e)
                return event

            flags = CGEventGetFlags(event)
            keycode = CGEventGetIntegerValueField(event, kCGKeyboardEventKeycode)
            fn_now = _trigger_is_active(keycode, flags)
            # GEÇİCİ TEŞHİS: hangi tuş hangi bayrakla geliyor, ölçelim.
            logger.info("DEBUG keycode=%s flags=0x%x fn_now=%s", keycode, flags, fn_now)
        ...
        # Bu üçü self'te tutulmak ZORUNDA. _start_event_tap bir daemon
        # thread'de kosuyor; yerel degisken olarak birakilirsa fonksiyon
        # biter bitmez Python bunlari cop topluyor. macOS tarafi Python
        # referansi saymadigi icin tap "active" yazip sessizce oluyor,
        # hicbir tus olayi gelmiyor. (Klasik PyObjC omur hatasi.)
        ...
        if tap is None:
            logger.info("CGEventTap failed, trying NSEvent fallback...")
    ...
    def _setup_nsevent_fallback(self):
        def add_monitor():
            ...
            # handler + monitor da self'te tutulmali (ayni omur hatasi).
            self._ns_handler = handler
            ...
            if monitor is None:
                ...
                logger.info("ERROR: NSEvent fallback also failed.")
                logger.error("Grant Accessibility permission to this app.")
    ...
    def _on_fn_press(self):
        ...
        self._recording_front_window = get_front_window_id()
        ...
        def _force_stop_recording():
            if self.is_recording:
                logger.warning("WATCHDOG: recording timeout (%ss), forcing stop", RECORDING_TIMEOUT_SEC)
                def release_on_main():
                    self._fn_held = False
                    self._on_fn_release()
    ...
    def _on_fn_release(self):
        ...
        threading.Thread(
            target=self._transcribe, args=(stream_ref, current_sid), daemon=True
        ).start()

    def _transcribe(self, stream_ref=None, session_id=0):
        import mlx_whisper
        ...
        if stream_ref:
            try:
                ...
                logger.error("Stream close error: %s", e)
        ...
        try:
            ...
            if duration < MIN_AUDIO_DURATION_SEC:
                logger.info(
                    "Too short after trim, skipping (effective=%.2fs, raw=%.2fs)",
            ...
            if os.path.exists(BENCHMARK_FLAG):
                try:
                    import shutil
                    os.makedirs(BENCHMARK_SAMPLE_DIR, exist_ok=True)
                    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    dst = os.path.join(BENCHMARK_SAMPLE_DIR, f"sample_{ts}.wav")
                    shutil.copy2(tmp, dst)
                    logger.info("Benchmark sample saved: %s", dst)
                except Exception as e:
                    logger.error("Benchmark save error: %s", e)
            ...
            if raw_text:
                ...
                processed = _enhance_with_ollama(processed)
                t_post = time.monotonic()
                logger.info("raw → %s", raw_text)
                logger.info("out → %s", processed)

                if not _session_valid():
                    logger.warning("Session %d expired, discarding result", session_id)
                    return
                ...
                sid = session_id  # capture for closures
                if same_app:
                ...
                logger.info(
                    ...
                    duration, asr_sec, rtf, t_post - t_asr_done,
...
def _setup_logging() -> None:
    ...
    class _PrivateRotatingHandler(RotatingFileHandler):
        def doRollover(self):
            super().doRollover()
            _set_private(self.baseFilename)

    fh = _PrivateRotatingHandler(
...
def main():
    ...
    if not os.path.exists(KEYWORDS_FILE):
        with open(KEYWORDS_FILE, "w") as f:
            # Orijinal varsayılan yazarın kendi finans jargonuydu
            # (NVIDIA, Tesla, S&P 500...) — bizim bağlamımızla alakasız,
            # boş bırakıyoruz. Fatih kendi terimlerini burada ekleyebilir.
            f.write("")
        _set_private(KEYWORDS_FILE)

    logger.info("Starting v2... (hold FN, or Right Command/Option, to talk)")
    logger.info("Keywords: %s", KEYWORDS_FILE)
    logger.info("History: %s", HISTORY_FILE)

    # GEÇİCİ TEŞHİS: Erişilebilirlik izni gerçekten BU çalışan process'e mi
    # atfedilmiş? macOS bunu imzalı executable'a göre değerlendiriyor —
    # .app'e izin vermek, launcher'ın exec ettiği asıl Python binary'sine
    # otomatik yansımayabilir.
    try:
        import ctypes
        ...
        logger.info(
            "DEBUG AXIsProcessTrusted=%s pid=%s exe=%s",
            trusted, os.getpid(), sys.executable,
        )
    except Exception as e:
        logger.info("DEBUG AXIsProcessTrusted check failed: %s", e)
